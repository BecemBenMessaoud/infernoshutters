...
const IMAGE_SIZE =
  'h-[280px] w-full max-w-[360px] overflow-hidden rounded-xl sm:h-[320px] sm:max-w-[400px] lg:h-[380px] lg:max-w-[460px]'

type CustomFabricationSectionProps = {
  embedded?: boolean
}

export function CustomFabricationSection({ embedded = false }: CustomFabricationSectionProps) {
  const [isAssessmentModalOpen, setIsAssessmentModalOpen] = useState(false)

  const content = (
    <>
      <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-x-12 lg:gap-y-8">
      ...
      <div className="mt-12 text-center">
        <button
          ...
          className="inline-block min-h-11 rounded-md bg-inferno-500 px-10 py-3 text-sm font-bold text-white transition hover:bg-inferno-600"
        >
  ...
  return (
    <>
      {embedded ? (
        <div id="contact-fabrication">{content}</div>
      ) : (
